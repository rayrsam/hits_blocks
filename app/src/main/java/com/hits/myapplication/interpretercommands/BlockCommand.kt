...
interface BlockCommand {
    ...
    private fun simpleOperation(left: String, right: String, operation: String): String {
        return when (operation) {
            ...
            "==" -> (left == right).toString()
            "!=" -> (left != right).toString()
            ">" -> ((Interpreter.convertVariable(left) as Comparable<Any>) > (Interpreter.convertVariable(
                right
            ) as Comparable<Any>)).toString()
    ...
    private fun parseExpression(expression: String): List<String> {
        ...
        for (i in result.indices) {
            ...
            if (Interpreter.getVar(result[i]) != null) result[i] =
                Interpreter.getVar(result[i]).toString()
            else if (result[i] == "-" && (i == 0 || (!result[i - 1].last()
                    .isLetterOrDigit() && result[i - 1].last() != ')'))
            ) result[i] = "~"
        }
        return result
    }
